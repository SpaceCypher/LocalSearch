...
### ✅ Task 21 — Rust ↔ Swift FFI (`src/ffi.rs`, `frontend/Sources/Backend/FFIBackend.swift`)
**Commit:** `feat(ffi): C ABI for Swift UI integration`

**What was built:**
- Added Task 21 C ABI in Rust:
  - `localsearch_query(query, results, count) -> i32`
  - `localsearch_free_results(results, count)`
  - `#[repr(C)] SearchResult { doc_id, path, score, snippet }`
- Implemented memory ownership contract:
  - Rust allocates result array and C strings
  - Swift consumes array via function pointer ABI
  - Swift calls `localsearch_free_results` to release all allocations
- Updated Swift backend bridge to load and call Task 21 symbols (`dlsym` + typed C function pointers)
- Removed JSON-string bridge dependency path in favor of direct C-struct interop
...
- `localsearch_free_results` frees both nested C strings and the result buffer to avoid leaks
